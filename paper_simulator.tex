...
\hline
\end{tabular}}
\caption{Module responsibilities and dependency graph (consumer $\to$ provider).}
\label{fig:arch}
\end{figure}
...
Five edge nodes are created with heterogeneous capacities: nodes 0, 1, 3 hold 9000\,MB each; nodes 2 and 4 hold 2700\,MB each. Each node is assigned a region $r_j = j \bmod N_r$, spreading the five nodes across the three regions $(0, 1, 2)$.
...
Identical to DEADLINE\_HEURISTIC except the $w_{\text{overlap}}$ term is dropped (flag \texttt{drop\_overlap} set to \texttt{True}). This ablation tests whether the soft cache-partitioning incentive in the overlap term is beneficial.
